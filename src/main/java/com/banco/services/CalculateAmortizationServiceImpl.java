...
public class CalculateAmortizationServiceImpl implements CalculateAmortizationService{
    ...
    @Override
    public List<LoanPayment> calculateAmortization(Loan loan) throws IllegalArgumentException {
        ...
        for(int i = 0; i < totalPayments; ++i) {
            BigDecimal interestPayment = remainingBalance.multiply(monthlyInterestRate).setScale(3, RoundingMode.HALF_UP);
            BigDecimal principalPayment = monthlyPayment.subtract(interestPayment).setScale(3, RoundingMode.HALF_UP);
            remainingBalance = remainingBalance.subtract(principalPayment).setScale(3, RoundingMode.HALF_UP);
            LoanPayment payment = new LoanPayment();
            payment.setPaymentNumber(i + 1);
            payment.setPaymentAmount(monthlyPayment.setScale(3, RoundingMode.HALF_UP));
    ...
    public BigDecimal calculateMonthlyPayment(BigDecimal loanAmount, BigDecimal monthlyInterestRate, int totalPayments) {
        ...
        return numerator.divide(denominator, 7, RoundingMode.HALF_UP).setScale(3, RoundingMode.HALF_UP);
    }
    @Override
